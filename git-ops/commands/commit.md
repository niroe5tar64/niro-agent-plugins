...
**このコマンドの唯一の責務は「コミットメッセージを生成してコミットすること」です。**

**使用するツールは git コマンド（status, diff, log, commit）のみです。**

以下の行動は**絶対に禁止**です：
- コミット後に実装を開始する
- コミット後に次のステップやタスクを実行する
- コミットした内容に関する作業を続行する
- TodoWriteツールを使用してタスクを作成する
- ファイルの編集や追加を行う
- コードレビューや改善提案を行う
- 変更内容に対する評価やアドバイスを提供する
- 次のアクション（push、PR作成など）を提案する
...
以下のコマンドを実行してステージ状況を把握する。
**並列実行が可能な環境では、1)〜4) を同時に実行してよい。**
...
**コミットが完了したら、このコマンドは即座に終了します。**

- コミット結果を簡潔に報告する（コミットID、変更ファイル数など）
- それ以外は**何も行わない**

このコマンドの役割は「コミットの作成」のみです。コミット後は**何もせず終了**してください。
